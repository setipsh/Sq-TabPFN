...
# 忽略 numpy 的 VisibleDeprecationWarning
...
param_grid_hyperopt['logistic'] = {
    ...
    , 'max_iter': hp.randint('max_iter', 50, 500)
    , 'fit_intercept': hp.choice('fit_intercept', [True, False])
    , 'C': hp.loguniform('C', -2, math.log(2.0))}  # 'normalize': [False],
...
## SVM
param_grid_hyperopt['svm'] = {'C': hp.choice('C', [0.1,1, 10]), 'gamma': hp.choice('gamma', ['auto']),'kernel': hp.choice('kernel', ['rbf', 'poly', 'sigmoid'])}
...
param_grid_hyperopt['catboost'] = {
    ...
    'random_strength': hp.choice('random_strength', [2, 3]),
    #'l2_leaf_reg': hp.choice('l2_leaf_reg', [3, 5]),
    'l2_leaf_reg': 3,
    'bagging_temperature': hp.uniform('bagging_temperature', 0.1, 1),
    'leaf_estimation_iterations': 5,
    #'leaf_estimation_iterations': hp.choice('leaf_estimation_iterations', [5, 10]),
    'iterations': hp.quniform('iterations', 100, 800, 100),
    'depth': 6,
...
param_grid_hyperopt['xgb'] = {
    ...
    'lambda': hp.loguniform('lambda', -2, 0),
    'gamma': hp.loguniform('gamma', -2, 0),
    'n_estimators': hp.randint('n_estimators', 100, 1000), # This is smaller than in paper
}
